...
const PlayersContent = dynamic(
  () => import("@/app/(game)/admin/_tabs/PlayersTab"),
  { loading: () => <PageLoader icon={<GameIcon name="settings" size={32} />} text="Loading Players..." /> }
);

const CharactersContent = dynamic(
  () => import("@/app/(game)/admin/_tabs/CharactersTab"),
  { loading: () => <PageLoader icon={<GameIcon name="fights" size={32} />} text="Loading Characters..." /> }
);

const MatchesContent = dynamic(
  () => import("@/app/(game)/admin/_tabs/MatchesTab"),
  { loading: () => <PageLoader icon={<GameIcon name="arena" size={32} />} text="Loading Matches..." /> }
);

const EconomyContent = dynamic(
  () => import("@/app/(game)/admin/_tabs/EconomyTab"),
  { loading: () => <PageLoader icon={<GameIcon name="gold" size={32} />} text="Loading Economy..." /> }
);

/* ────────────────── Tab definitions ────────────────── */

type TabId = "dev" | "players" | "characters" | "matches" | "economy" | "balance" | "design";
...
const TABS: TabDef[] = [
  ...
  { id: "players", label: "Players", icon: "settings" },
  { id: "characters", label: "Characters", icon: "fights" },
  { id: "matches", label: "Matches", icon: "arena" },
  { id: "economy", label: "Economy", icon: "gold" },
