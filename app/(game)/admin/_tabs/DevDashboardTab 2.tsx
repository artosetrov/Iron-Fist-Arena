...
type StatCardProps = { icon: React.ReactNode; label: string; value: string | number; sub?: string };
  ...
  >
    {loading && !data ? (
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
        ...
        <StatCard
          icon={<GameIcon name="fights" size={28} />}
          label="Characters"
          value={data.characters}
          sub={`Avg level: ${data.avgLevel} | Max: ${data.maxLevel}`}
        />
        <StatCard
          icon={<GameIcon name="leaderboard" size={28} />}
          label="PvP Matches"
          value={data.pvpMatches}
          sub={`Avg rating: ${data.avgPvpRating} | Top: ${data.maxPvpRating}`}
        />
        <StatCard icon={<GameIcon name="rogue" size={28} />} label="Items in Catalog" value={data.items} />
        <StatCard icon={<GameIcon name="dungeons" size={28} />} label="Dungeon Clears" value={data.dungeonProgress} />
...
const AuditSection = () => (
  ...
  >
    <div className="max-h-80 overflow-y-auto -m-5 p-5">
...
const ApiPingSection = ({
  ...
  <GameSection
    title={`API Endpoints (${API_ENDPOINTS.length})`}
    headerRight={
      <GameButton
        variant="secondary"
        size="sm"
...
export default DevDashboardTab;
